...
/// Enqueue a runnable task into the global FIFO dispatch queue.
#[rex_sched_ext(callback = "enqueue")]
fn simple_enqueue(obj: &sched_ext, p: &TaskStruct, enq_flags: u64) {
    obj.scx_bpf_dsq_insert(p, SCX_DSQ_GLOBAL, SCX_SLICE_DFL, enq_flags);
}

/// Called when the local DSQ is empty. Pull a task from the global DSQ.
...
fn simple_dispatch(
    ...
    obj.scx_bpf_consume(SCX_DSQ_GLOBAL);
}

/// Called once when the scheduler is loaded.
#[rex_sched_ext(callback = "init")]
fn simple_init(_obj: &sched_ext) -> i32 {
    0
}
